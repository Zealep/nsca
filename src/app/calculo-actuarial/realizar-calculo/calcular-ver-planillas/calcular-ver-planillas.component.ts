...
export class CalcularVerPlanillasComponent {
  ...
  verReportes() {
    ...
    let solicitudReport: BandejaSolicitud = {
      idSolicitud: this.solicitud.idSolicitud.toString(),
      desTipoSolicitud: this.solicitud.desTipoSolicitud,
      anhoCalculo: this.solicitud.anhoCalculo,
      descEstadoSolicitud: this.solicitud.descEstadoSolicitud,
      descSolicitud: '',
      fechRecepcion: '',
      fechCrea: '',
      estadoSolicitud: '',
    ...
    modalRef.componentInstance.solicitud = solicitudReport
  ...
  aprobar(codTipPla: string, desTipPla: string) {
    ...
    this.spinnerService.show()
    this.solicitudService.aprobarPlanilla(this.solicitud.tipoSolicitud, this.solicitud.idSolicitud, codTipPla, this.usuario, '127.0.0.1')
      .pipe(catchError(error => {
        this.spinnerService.hide()
        this.toastService.show(error, { classname: 'bg-danger text-white', delay: 3000, icon: 'ban' })
        return EMPTY
